...
namespace CyanAir {
    ...
    export class CyanAir {
        ...
        //Parámetros:
        //  * archivoComponente: nombre del archivo que contiene el componente, incluyendo la ruta.
        //  * idNodoPadre: nodo al cual se añade como nodo hijo el nuevo componente.
        //  * clases: clases a añadir al nuevo nodo cuando se cree (solo si el componente es de tipo HTML). Deben separarse por espacios si hay más de una clase.
        public static Import(archivoComponente: string, idNodoPadre: string = '', clases: string = '') {
            function extraerCaracteresAlfabeticos(texto: string) {
                let resultado = '';
                for (let i = 0; i < texto.length; i++) {
                    let caracter = texto[i];
                    if (caracter.match(/[a-zA-Z0-9-\_]/)) {
                        resultado += caracter;
                    }
                }
                return resultado;
            ...
            let componentId = String(archivoComponente.split(".").slice(-2, -1));
            ...
            xhr.onreadystatechange = () => {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {
                        ...
                        if (idNodoPadre != "") {
                            nodoPadre = document.getElementById(idNodoPadre);
                        } else {
                            nodoPadre = document.getElementsByTagName("BODY")[0];
                        }
                        ...
                        if (codigoHTMLCSS != '') {
                            ...
                            if (clases != '') {
                                nodoNuevoHTMLCSS.className = clases;
                            }
                            nodoNuevoHTMLCSS.innerHTML = codigoHTMLCSS;
                            nodoPadre.appendChild(nodoNuevoHTMLCSS);
                        ...
                        if (codigoSCRIPT != '') {
                            ...
                            nodoPadre.appendChild(nodoNuevoSCRIPT);
                        ...
                        CyanAir.Talk(componentId, componentId + "_OnLoaded", archivoComponente);
                    } else {
                        try {
                            window[componentId + "_OnLoadingError"].apply(this);
                        } catch(e) {}
                        console.log("Error: object " + archivoComponente + " could not be loaded.");
                        CyanAir.Talk(componentId, componentId + "_OnLoadingError", archivoComponente);
                    }
                }
            }
            xhr.open('GET', archivoComponente, false);
            xhr.send();
        }
    ...
    export function Import(archivoComponente: string, idNodoPadre: string = '', clases: string = '') {
        CyanAir.Import(archivoComponente, idNodoPadre, clases);
    }
